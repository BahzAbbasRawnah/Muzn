...
Future<void> _onLoadStudents(LoadStudents event, Emitter<StudentState> emit) async {
  emit(StudentLoading());
  try {
    final circleData = await studentController.fetchCircleWithStudentById(event.circleId);
  ...
  Future<void> _onAddStudent(AddStudent event, Emitter<StudentState> emit) async {
    try {
      await studentController.addStudent(event.student.toMap());
      add(LoadStudents(1));
    } catch (e) {
      emit(StudentError(e.toString()));
    }
  ...
  Future<void> _onEditStudent(EditStudent event, Emitter<StudentState> emit) async {
    try {
